...
def load_lm(lm_name: str):
    tok = AutoTokenizer.from_pretrained(lm_name, use_fast=True)

    # Some LMs (OPT/Pythia) may not have an explicit pad token; set it to eos to be safe.
...
@torch.no_grad()
def lex_entropy(text: str, tok, lm, max_len=512) -> float:
...
parser.add_argument("--lm_model", default="gpt2",
                    choices=["gpt2", "gpt2-medium", "facebook/opt-350m", "EleutherAI/pythia-410m", "openai-community/gpt2-xl"])
args = parser.parse_args()

_tok, _lm = load_lm(args.lm_model)
...
for txt_path in tqdm(Path("data").glob("*/**/*.txt"), unit="clip"):
    ...
    entropy_val = lex_entropy(raw, _tok, _lm)
...
safename = (args.lm_model).replace("/", "_").replace("\\", "_").replace(":", "").replace(".", "_")

out_csv = f"text_features_lexical__{safename}.csv"
